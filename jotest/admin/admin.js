...
function DefineForms() {
	...
	$('input, textarea').each(function(i) {
		...
		if(!$(this).attr('placeholder') && $(this).attr('title')) { $(this).attr('placeholder',$(this).attr('title')); }
...
function InputControl(obj,reurlctrl) {
	if($(obj)) {
		...
		if($(obj).attr('type')!='submit' && $(obj).attr('type')!='reset' && $(obj).attr('type')!='button') {
			...
			var filter = false;
			if(!$(obj).val() && $(obj).val()!="") { return false; }
...
function AddAlert(type,text,func) {
	...
	var randId = 'outAlert'+Math.floor(Math.random() * (999999 - 100000 + 1) + 100000);
		...
		$('.SiteMaxWidth').prepend('<div class="outAlert" id="'+randId+'" style="opacity:0;"><span class="shadow"><span class="text '+type+'"><span class="close" onclick="RemoveAlert(\''+randId+'\');"></span><pre><span class="image"></span>'+text+'</pre><br/><input class="button button'+randId+'" type="button" onclick="RemoveAlert(\''+randId+'\');" value=" OK " /></span></span></div>');
		...
		$('.SiteMaxWidth').prepend('<div class="outAlert" id="'+randId+'" style="opacity:0;"><span class="shadow"><span class="text '+type+'"><span class="close" onclick="RemoveAlert(\''+randId+'\');"></span><pre><span class="image"></span>'+text+'</pre><br/><center><input class="ok_button ok'+randId+'" type="button" onclick="'+(func)+'" value=" Igen! " /><input class="button no_button no'+randId+'" type="button" onclick="RemoveAlert(\''+randId+'\');" value=" Nem! " /></center></span></span></div>');
		setTimeout("$('#"+randId+"').animate({'opacity':'1'},300);",500);
