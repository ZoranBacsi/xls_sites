...
if(count($_private)>0 AND $_private[0]>0 AND !$_private_enable) {
	...
	if(!$_ENV['SzmenuType'] OR $_ENV['SZMENU']['szm_tipus']=='N') {
		...
		if(mysqli_num_rows($con)>0) {
			...
			while($sor = mysqli_fetch_array($con,MYSQLI_ASSOC)) {
				$_links[] = '<li><a href="'.(($_ENV['SZMENU']['szm_tipus']!='T')?(MenuFa($sor['szm_id'])):('#szm_'.$sor['szm_reurl'])).'">'.($sor['szm_link']).'</a></li>';
				if(strlen($sor['szm_text'])>3) {
					$_texts[] = '<div class="szMenuMainTextListItem'.(($_ENV['SZMENU']['szm_tipus']=='R')?(' szMenuShort'):('')).'" id="szm_'.($sor['szm_reurl']).'">'.(out_html($sor['szm_text'])).''.(($_ENV['SZMENU']['szm_tipus']=='R')?('<div class="szMenuBovebben"><a href="'.(MenuFa($sor['szm_id'])).'">Bővebben</a></div>'):('')).'</div>';
				}
				if($_ENV['SZMENU']['szm_tipus']=='S') {
					$_jsList[] = '
						<div class="szMenuRowJS">
							<h3 class="szMenuTitleJS"><span class="arrow"></span>'.($sor['szm_link']).'</h3>
						...
						$(".szMenuTitleJS").click(function() {
							...
							$(".szMenuTextJS").each(function() {
								if(!($(this).attr("opening")) || $(this).css("display")=="block") {
									...
									$("html, body").animate({
										scrollTop: (($("#nav").offset().top-100<=$("html, body").scrollTop())?($("#nav").offset().top):(0))
									}, 1000);
								}
	...
	echo (($_ENV['SZPAGE']['szp_text'])?('<div class="szMenuFullText">'.(out_html($_ENV['SZPAGE']['szp_text'])).'</div>'):(''));
}
