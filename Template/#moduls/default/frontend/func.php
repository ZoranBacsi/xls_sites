...
if($_ENV['SiteStart']!='CSS' AND $_ENV['SiteStart']!='JS') {
	...
	$_ENV['SITE']['REURL'] = end($_ENV['REURL']);
		...
		while($sor=mysqli_fetch_array($con,MYSQLI_ASSOC)) {
			if(strstr($s,'#{forms:'.($sor['form_id']).'}')) {
				$s = preg_replace(
					...
					'<span class="forms" id="forms_'.($sor['form_id']).'" style="width:\\1px;min-height:\\2px;"><script type="text/javascript">$(document).ready(function(){setTimeout(function(){LoadPage("?formsload='.($sor['form_id']).((is_array($_GET))?(ReGet($_GET)):('')).((is_array($_POST))?(ReGet($_POST)):('')).'");},300);});</script></span>',
					$s
				);
				$s = preg_replace(
					"/\<iframe style\=\"([0-9a-z.\#\-\:\;\ \(\)\.\,]+)\" src\=\"\#\{forms\:".($sor['form_id'])."\}\" width\=\"([0-9]+)\" height\=\"([0-9]+)\"\>\<\/iframe\>/i",
					'<span class="forms" id="forms_'.($sor['form_id']).'" style="\\1width:\\2px;min-height:\\3px;"><script type="text/javascript">$(document).ready(function(){setTimeout(function(){LoadPage("?formsload='.($sor['form_id']).((is_array($_GET))?(ReGet($_GET)):('')).((is_array($_POST))?(ReGet($_POST)):('')).'");},300);});</script></span>',
					$s
				);
			}
		}
							...
							$out .= '
								<p id="fr'.($row['fr_id']).'_row">
									<span class="row">
										<span class="labelCell"><label for="fr'.($row['fr_id']).'">'.($row['fr_label']).':</label></span>
										<span class="valusCell">
											<input type="text" class="'.($row['fr_type']).' '.(($row['fr_required'])?('required'):('')).'" id="fr'.($row['fr_id']).'" name="fr'.($row['fr_id']).'" value="'.(in_text($_REQUEST['fr'.$row['fr_id']])).'" '.(($row['fr_minlength'])?('minlength="'.($row['fr_minlength']).'"'):('')).' '.(($row['fr_maxlength'])?('maxlength="'.($row['fr_maxlength']).'"'):('')).' style="width:90px;" readonly />
											<span class="formsInputError">{formsInputError}</span>
										</span>
									</span>
									<script type="text/javascript">
										setTimeout(function() {
											$(\'#fr'.($row['fr_id']).'\',$(\'#forms'.($sor['form_id']).'\')).hover(function(){
												$(this).datepicker(\'destroy\');
												$(this).datepicker({'.((count($vals)>0)?(join(',',$vals)):('')).'});
											});
											$(\'#fr'.($row['fr_id']).'\',$(\'#forms'.($sor['form_id']).'\')).datepicker({'.((count($vals)>0)?(join(',',$vals)):('')).'});
										},100);
									</script>
								</p>';
							$_out_js .= '
								...
								<p id="fr'.($row['fr_id']).'_row">
									<span class="row">
										...
										<span class="valusCell">
											...
											<img src="/images/refresh.png" onclick="$(\'#captcha_kep_'.($row['fr_id']).'\').attr(\'src\',\'/captcha.png?r=\'+Math.random());" style="cursor:pointer;float:right;margin-top:-40px;" />
										</span>
										<span class="formsInputError">{formsInputError}</span>
									</span>
								</p>';
	...
	if(substr(strtolower($_ENV['CONF']['sysSiteFejlesztes']),0,1)=='i') {
		if($_ENV['SiteStart']!='AJAX' AND $_ENV['SiteStart']!='JSON') {
			...
			$_SESSION['sysSiteFejlesztes']=1;
		}
		// if($_SERVER['REQUEST_URI']=='/' AND !($_COOKIE['sysAlert'])) {
		if(!($_COOKIE['sysAlert'])) {
			setcookie('sysAlert', 1, (time() + 28800), "/");
			$_ENV['SITE']['sysAlert'] = '
