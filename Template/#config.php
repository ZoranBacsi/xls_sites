...
/* Cookie Policy 2015 Okt. 01. */
if($_COOKIE['cookiePolicy']) { session_start(); } else { if(isset($_SERVER['HTTP_COOKIE'])) { $cookies = explode(';', $_SERVER['HTTP_COOKIE']); foreach($cookies as $cookie) { $parts = explode('=', $cookie); $name = trim($parts[0]); setcookie($name, '', 0); setcookie($name, '', 0, '/'); } } unSet($_SESSION); unSet($_COOKIE); $_SESSION = Array(); $_COOKIE = Array(); }
