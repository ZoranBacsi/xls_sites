...
function is_mobile($mobil=false) {
	return (((preg_match('/(alcatel|amoi|android|avantgo|blackberry|benq|cell|cricket|docomo|elaine|htc|iemobile|iphone|ipad|ipaq|ipod|j2me|java|midp|mini|mmp|mobi|motorola|nec-|nokia|palm|panasonic|philips|phone|playbook|sagem|sharp|sie-|silk|smartphone|sony|symbian|t-mobile|telus|up\.browser|up\.link|vodafone|wap|webos|wireless|xda|xoom|zte)/i', $_SERVER['HTTP_USER_AGENT']) OR ( substr($_SERVER['HTTP_HOST'], 0, 2) == 'm.' AND !$mobil )) AND $_ENV['CONF']['MOBIL']) ? (true) : (false));
}
...
function is_home() { return ((($_ENV['REURL'][1]=='szmenu' AND $_ENV['REURL'][2]=='index') OR ( substr($_SERVER['REQUEST_URI'],0,7) == '/index.' OR $_SERVER['REQUEST_URI'] == '/' ))?(true):(false)); }
function isHome() { return is_home(); }
