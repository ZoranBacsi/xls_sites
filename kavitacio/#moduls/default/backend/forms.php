				...
				<tr id="forms_row_'.($n).'" '.(($row['a']==0)?(''):('class="hidden"')).' style="display:'.(($row['a']==0)?('table-row'):('none')).'">
					...
					<td class="forms_row_optiongroup1">
						...
						<div class="selects" style="display:'.(($optg1=='selects')?('table-row'):('none')).';">
							...
							<textarea class="typeText required" name="'.(($row['fr_id']>0)?(''):('new_')).'fr_value['.(($row['fr_id'])?($row['fr_id']):($row['na'])).']" id="fr_value_'.($n).'" style="width:200px;height:70px;white-space:nowrap;" wrap="off">'.($row['fr_value']).'</textarea>
						</div>
					</td>
					<td>
						<p>
