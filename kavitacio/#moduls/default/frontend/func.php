		...
		while($sor=mysqli_fetch_array($con,MYSQLI_ASSOC)) {
			if(strstr($s,'#{maps:'.($sor['map_id']).'}')) {
				$s = preg_replace(
					...
					'<span class="gmaps" id="gmaps'.($sor['map_id']).'" style="width:\\1px;height:\\2px;"><script type="text/javascript">$(document).ready(function(){loadGoogleMaps({"ID":"gmaps'.($sor['map_id']).'","lati":"'.($sor['map_lati']).'","longi":"'.($sor['map_longi']).'","cimke":"'.($sor['map_cimke']).'","irsz":"'.($sor['map_irsz']).'","varos":"'.($sor['map_varos']).'","utca":"'.($sor['map_utca']).'","tel":"'.($sor['map_tel']).'","fax":"'.($sor['map_fax']).'","email":"'.($sor['map_email']).'","web":"'.($sor['map_web']).'"});});</script></span>',
					$s
				);
				$s = preg_replace(
					"/\<iframe style\=\"([0-9a-z.\#\-\:\;\ \(\)\.\,]+)\" src\=\"\#\{maps\:".($sor['map_id'])."\}\" width\=\"([0-9]+)\" height\=\"([0-9]+)\"\>\<\/iframe\>/i",
					'<span class="gmaps" id="gmaps'.($sor['map_id']).'" style="\\1width:\\2px;height:\\3px;"><script type="text/javascript">$(document).ready(function(){loadGoogleMaps({"ID":"gmaps'.($sor['map_id']).'","lati":"'.($sor['map_lati']).'","longi":"'.($sor['map_longi']).'","cimke":"'.($sor['map_cimke']).'","irsz":"'.($sor['map_irsz']).'","varos":"'.($sor['map_varos']).'","utca":"'.($sor['map_utca']).'","tel":"'.($sor['map_tel']).'","fax":"'.($sor['map_fax']).'","email":"'.($sor['map_email']).'","web":"'.($sor['map_web']).'"});});</script></span>',
					$s
				);
			}
		}
		...
		while($sor=mysqli_fetch_array($con,MYSQLI_ASSOC)) {
			if(strstr($s,'#{forms:'.($sor['form_id']).'}')) {
				$s = preg_replace(
					...
					'<span class="forms" id="forms_'.($sor['form_id']).'" style="width:\\1px;min-height:\\2px;"><script type="text/javascript">$(document).ready(function(){setTimeout(function(){LoadPage("?formsload='.($sor['form_id']).'");},300);});</script></span>',
					$s
				);
				$s = preg_replace(
					"/\<iframe style\=\"([0-9a-z.\#\-\:\;\ \(\)\.\,]+)\" src\=\"\#\{forms\:".($sor['form_id'])."\}\" width\=\"([0-9]+)\" height\=\"([0-9]+)\"\>\<\/iframe\>/i",
					'<span class="forms" id="forms_'.($sor['form_id']).'" style="\\1width:\\2px;min-height:\\3px;"><script type="text/javascript">$(document).ready(function(){setTimeout(function(){LoadPage("?formsload='.($sor['form_id']).'");},300);});</script></span>',
					$s
				);
			}
		}
				...
				while($row = mysqli_fetch_array($rCon)) {
					...
					switch ($row['fr_type']) {
						...
						case 'typeTexta':
							$out .= '
								<p id="fr'.($row['fr_id']).'_row">
									<span class="row"><label for="fr'.($row['fr_id']).'">'.($row['fr_label']).':</label></span>
									<span class="row">
	...
	function FormsControl($formid) { $out = $_out_js = '';
		...
		if($sor=mysqli_fetch_array($con,MYSQLI_ASSOC)) {
			...
			while($row = mysqli_fetch_array($rCon)) {
				...
				AND ( (!($iRequired) AND $iLength==0) OR $iFilter )
				) {
		...
		if($sor=mysqli_fetch_array($con,MYSQLI_ASSOC)) {
			...
			while($row = mysqli_fetch_array($rCon)) {
				...
				if($iValue) {
					...
					if($row['fr_type']!='typeCaptcha') { $mailDataRows .= '<p><b>'.($row['fr_label']).':</b> '.($iValue).'</p>'; }
