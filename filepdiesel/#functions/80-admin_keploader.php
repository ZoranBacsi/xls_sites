...
class KepUP {
	...
	static function AdminForm($id,$limit,$cimke,$sorszam,$albumkep,$kepfelcimke=false) { $OUT=NULL; $i=0;
		...
		if($kepfent<$limit) {
				$OUT .= '
				<tr class="fejlec">
					<th colspan="2" align="left">'.(($kepfelcimke)?($kepfelcimke):('Képfeltöltés')).'</th>
				</tr>
				<tr>
					<td colspan="2">
						<div class="helpBox">
			...
			$OUT .= '
							<script type="text/javascript">
								function KepLoadForm(obj,n) {
									...
									if(allowSubmit) {
										$("#KepUp_"+n).css("display","inline-block");
									} else {
										obj.value="";
			...
			for($a=0;$a<count($_FILES['kepupload']['tmp_name']);$a++) {
				if(strstr($_FILES['kepupload']['type'][$a],'image')) {
					...
					if($_ENV['KepUploadMode']($_FILES['kepupload']['tmp_name'][$a], DOCUMENT_ROOT.$upload_file_eredeti)){
						...
						mysqli_query($_ENV['MYSQLI'],'INSERT INTO `kepek` ( `kep_mid`, `kep_sorsz`, `kep_cimke`, `kep_cimke_en`, `kep_cimke_de`, `kep_cimke_it`, `kep_cimke_ru`, `kep_cimke_ro`, `kep_cimke_sk`, `kep_cimke_jp`, `kep_eredeti`, `kep_nagy`, `kep_kozepes`, `kep_kicsi`, `kep_type`, `kep_name`, `kep_date` ) VALUES ( "'.($id).'", "'.($uj_sorsz).'", "'.(in_text($_REQUEST['ujkepcimke'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_en'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_de'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_it'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_ru'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_ro'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_sk'][$a])).'", "'.(in_text($_REQUEST['ujkepcimke_jp'][$a])).'", "'.(in_text($upload_file_eredeti)).'", "'.(in_text($upload_file_nagy)).'", "'.(in_text($upload_file_norm)).'", "'.(in_text($upload_file_kics)).'", "'.(in_text($_FILES['kepupload']['type'][$a])).'", "'.(in_text($_FILES['kepupload']['name'][$a])).'", now() )');
						print(mysqli_error($_ENV['MYSQLI']));
			...
			if(is_array($_REQUEST['kepid'])) {
				for($a=0;$a<count($_REQUEST['kepid']);$a++) {
					mysqli_query($_ENV['MYSQLI'],'
						UPDATE `kepek`
						SET
							`kep_sorsz`="'.(in_text($_REQUEST['kep_sorsz'][$a])).'",
							`kep_cimke`="'.(in_text($_REQUEST['kepcimke'][$a])).'",
							`kep_cimke_en`="'.(in_text($_REQUEST['kepcimke_en'][$a])).'",
							`kep_cimke_de`="'.(in_text($_REQUEST['kepcimke_de'][$a])).'",
							`kep_cimke_it`="'.(in_text($_REQUEST['kepcimke_it'][$a])).'",
							`kep_cimke_ru`="'.(in_text($_REQUEST['kepcimke_ru'][$a])).'",
							`kep_cimke_ro`="'.(in_text($_REQUEST['kepcimke_ro'][$a])).'",
							`kep_cimke_sk`="'.(in_text($_REQUEST['kepcimke_sk'][$a])).'",
							`kep_cimke_jp`="'.(in_text($_REQUEST['kepcimke_jp'][$a])).'"
						WHERE `kep_id`="'.($_REQUEST['kepid'][$a]).'"
						LIMIT 1
					');
				}
			}
